...
import { pdfs } from "../../../../assets/json/pdfs-blogs";
...
const Infografia = ({ location = null }) => {
  ...
  const download = pdfs.find(
    (item) => item.media_id === currentMediaPlaying.mediaId
  );
    ...
    <div className="infografiapop">
      <div className="infografiapop_header-section">
        <div className="infografiapop_header-text">
          ...
          <p>{currentMediaPlaying.mediaText1}</p>
        </div>
        <div className="infografiapop_close-icon" onClick={handleClosInfografy}>
      ...
      <div className="infografiapop_btns-box">
        ...
        <a
          href={currentMediaPlaying.mediaTitle != "BLOG" ? currentMediaPlaying.filePath : download.download}
          download={currentMediaPlaying.mediaTitle != "BLOG" ? currentMediaPlaying.filePath : download.download}
        // href={currentMediaPlaying.filePath }
        // download={currentMediaPlaying.filePath }
          className="infografiapop_download-icon infografiapop_icon"
          target="_blank"
        >
        ...
        <div className="infografiapop_share-icon infografiapop_icon">
          ...
          {showShareBox && (
            <div className="infografiapop_share-icons-box">
              ...
              />
              <a
                href={`https://www.facebook.com/sharer/sharer.php?u=${LOCAL_URL}/${currentMediaPlaying.mediaId}`}
                target="_blank"
                rel="noreferrer"
                className="share-icon animated rotateIn"
              >
                <img src={FacebookIcon} alt="facebook icon" />
              </a>
              <a
                href={`https://api.whatsapp.com/send?text=${LOCAL_URL}/${currentMediaPlaying.mediaId}`}
                target="_blank"
                rel="noreferrer"
                className="share-icon animated rotateIn"
              >
              ...
              <a
                href={`https://www.linkedin.com/sharing/share-offsite/?url=${LOCAL_URL}/${currentMediaPlaying.mediaId}`}
                target="_blank"
                rel="noreferrer"
                className="share-icon animated rotateIn"
              >
              ...
              <div
                onClick={() =>
                  hanldeCopyClipboard(
                    `${LOCAL_URL}/${currentMediaPlaying.mediaId}`
                  )
                }
                className="share-icon animated rotateIn"
              >
      ...
      {currentMediaPlaying.mediaTitle === "BLOG" ? (
        <iframe
        ...
        <Lightbox image={currentMediaPlaying.filePath} title="1/1" />
      )}
    </div>
  );
};
